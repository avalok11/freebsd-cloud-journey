...
**Дата:** 2026-08-06 (v6 — два репозитория, Gitea вместо GitLab)
...
### Локальный стенд (на твоём Mac M4)
...
Твой Mac M4 — **управляющая консоль**, не часть кластера. На нём:
- Установлен Ansible.
- Хранятся SSH-ключи.
...
- Подключение к Selectel-серверам идёт по SSH на их публичный IP — твой IP не важен.
- Из Selectel к тебе домой никто не ходит.
...
- **Шеллы: разделение ролей.** Login shell = tcsh/csh (твоя зона комфорта, исторически нативно для FreeBSD). **Скрипты автоматизации = bash (POSIX sh) + Python.** Никаких скриптов на tcsh: несовместимо с Ansible, нечитаемо в любых доках, в мире автоматизации так никто не пишет. Python — для всего, что сложнее 20 строк логики. Shebang в скриптах: `#!/bin/sh` или `#!/usr/bin/env bash` или `#!/usr/bin/env python3`, **никогда `#!/bin/tcsh`**.
...
2. На Mac M4: `git clone git@github.com:<твой-ник>/freebsd-cloud-journey.git ~/projects/freebsd-cloud-journey`.
...
6. **Скажи мне путь к репозиторию** — я смогу работать с файлами напрямую, читать, редактировать, коммитить.
...
|---|---|---|
| Интерактивная работа | tcsh/csh | Родное для FreeBSD, твоя зона комфорта |
